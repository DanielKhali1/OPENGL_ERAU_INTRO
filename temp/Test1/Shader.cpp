...
Shader::Shader(const char* vertexShaderSource,const char* fragmentShaderSource)
{
	...
	//checkStatus(vertexShaderIndex, infoLog);
	...
	//checkStatus(fragmentShaderIndex, infoLog);
	...
	//checkStatus(this->shaderProgram, infoLog);

	glDeleteProgram(vertexShaderIndex);
	glDeleteProgram(fragmentShaderIndex);
}

void Shader::checkStatus(unsigned int shaderIndex, char * infoLog)
{
	int success = 1;
